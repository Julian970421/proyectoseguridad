...
public class Vigenere {
	...
	public void cifrar() //Genera cifrado
	{
		...
		for(int cont=0;cont<mensaje.length;cont++)
		{
			...
			i=(int)this.mensaje[cont]-0; //restamos 97 para pasar de codigo ascii a un numero entero
			j=(int)this.clave[cont]-0;
	...
	private char[] generarAbecedario()
	{
		...
		for(int i= 48; i<= 57;i++)  //en ascii a=97 y z=122
		{
			abc[posicion]=(char)i;
                        posicion++;

		}
                for(int i= 97; i<= 122;i++)  //en ascii a=97 y z=122
		{
			abc[posicion]=(char)i;
                        posicion++;
